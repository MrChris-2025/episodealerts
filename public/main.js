...
(function() {
  function initApp() {
    // 1. Ensure Parse is loaded before execution
    if (typeof Parse === 'undefined') {
      console.error("Parse SDK is not loaded. Please make sure the Parse SDK <script> tag is loaded before main.js.");
      return;
    }
    ...
    // DOM refs (queried safely once DOM is ready)
    ...
    function attachLiveSearchHandlers() {
      ...
      if (genreSelect) {
        genreSelect.addEventListener("change", () => { scheduleLiveSearch(); });
      }
      if (yearSelect) {
        yearSelect.addEventListener("change", () => { scheduleLiveSearch(); });
      }
    }

    // Wire subscribe/test buttons safely
  ...
  // Ensure DOM is fully constructed before execution starts
