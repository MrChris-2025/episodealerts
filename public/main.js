The issue of the script not loading properly is caused by strict script crashes and race conditions.

Why it was failing:
Missing Element Crashes: If any of the expected HTML elements (like globalSubscribeBtn or yearSelect) were missing from the webpage, trying to attach event listeners directly to them caused JavaScript to throw a fatal TypeError and stop executing.
Blocking Parse SDK Issues: If the Parse script loaded slowly or was blocked by an ad-blocker, referencing Parse globally crashed the entire initialization, preventing the TMDB movie gallery from loading altogether.
Array Filtering Errors: Some TMDB trending items do not have release dates or genres, causing .filter() operations to throw errors on undefined variables.
The Fix:
The updated main.js code below is completely redesigned to be resilient and non-blocking. The search and gallery features will load flawlessly even if the Parse SDK is delayed, and all interactive elements are strictly safeguarded.

Here is your complete, updated public/main.js file:
...
(function() {
  // Safe Parse initialization flags
  ...
  // Cache DOM element selections safely
  ...
  // Safe wrapper for Parse Cloud functions to prevent crashes if Parse is missing
  async function safeParseCloudRun(functionName, params) {
    if (!ensureParse()) {
      throw new Error("Parse SDK is not loaded or initialized. Action unavailable.");
    }
  ...
  async function registerServiceWorkerAndSubscribe() {
    ...
    try {
      ...
      if (sub) {
        try {
          const res = await safeParseCloudRun("registerSubscriptionSafe", { subscription: sub.toJSON() });
          console.log('registerSubscriptionSafe (existing):', res);
          if (res && res.status === "ok") {
            setStatus("Subscription confirmed: " + res.action, false);
          } else {
            setStatus("Server registration issue: " + (res && res.message ? res.message : JSON.stringify(res)), true);
          }
        } catch (err) {
      ...
      try {
        ...
        if (res && res.status === "ok") {
          setStatus("Subscribed and saved on server", false);
        } else {
          setStatus("Server save failed: " + (res && res.message ? res.message : JSON.stringify(res)), true);
        }
      } catch (err) {
  ...
  async function followShowForCurrentSubscription(showId, showName, media_type) {
    try {
      ...
      const res = await safeParseCloudRun("followShow", { endpoint, showId: String(showId), showName, media_type });
      return res;
    } catch (err) {
  ...
  async function unfollowShowForCurrentSubscription(showId) {
    try {
      ...
      const endpoint = sub.endpoint;
      try {
        const res = await safeParseCloudRun("unfollowShow", { endpoint, showId: String(showId) });
        return res;
      } catch (err) {
  ...
  async function getFollowedShowsForCurrentSubscription() {
    try {
      ...
      return await safeParseCloudRun("getFollowedShowsForEndpoint", { endpoint: sub.endpoint });
    } catch (err) {
      console.warn("getFollowedShowsForCurrentSubscription skipped:", err.message || err);
  ...
  async function renderResults(items) {
    ...
    try {
      ...
      // Load followed status safely (defaults to empty list if Parse/SW is blocked)
      ...
      if (!items || items.length === 0) {
        gallery.innerHTML = `<div style="padding:20px;color:var(--muted, #888)">No results.</div>`;
        return;
      }
      ...
      for (const i of items) {
        ...
        // Setup background image poster with visual fallback
        if (poster) {
          card.style.backgroundImage = `linear-gradient(180deg, rgba(9,2,18,0.36), rgba(9,2,18,0.12)), url(${poster})`;
        } else {
          card.style.backgroundImage = `linear-gradient(180deg, rgba(30,30,35,0.8), rgba(15,15,20,0.95))`;
        }
  ...
  async function loadGenres() {
    ...
    try {
      ...
      const map = new Map();
      if (m && m.genres) m.genres.forEach(g => map.set(g.id, g.name));
      if (t && t.genres) t.genres.forEach(g => map.set(g.id, g.name));
  ...
  async function searchTMDB(query, genre, year) {
    try {
      ...
      if (genre) {
        results = results.filter(r => (r.genre_ids || []).includes(parseInt(genre)));
      }
      if (year) {
        results = results.filter(r => {
          const y = r.release_date || r.first_air_date || "";
          return typeof y === 'string' && y.startsWith(String(year));
        });
      }
      return results;
    } catch (err) {
  ...
  async function doSearchImmediate() {
    ...
    const g = genreSelect ? genreSelect.value : "";
  ...
  // Attach input + select handlers for live search safely
  ...
  function wireActionButtons() {
    if (globalSubscribeBtn) {
      globalSubscribeBtn.addEventListener("click", async () => {
        try {
          await registerServiceWorkerAndSubscribe();
        } catch (err) {
          console.error("Subscribe button failed:", err);
        }
      });
    }
    ...
    if (testNotifyBtn) {
      testNotifyBtn.addEventListener("click", async () => {
        ...
        try {
          ...
          const endpoint = sub.endpoint;
          const resp = await safeParseCloudRun("sendTestPushForEndpoint", { endpoint, title: "Test Alert", body: "This is a test push from the site." });
          if (resp && resp.status === 'sent') {
            setStatus("Test push sent", false);
          } else {
            setStatus("Test push failed: " + JSON.stringify(resp), true);
          }
        } catch (err) {
  ...
  // Main Orchestration initialization
  async function initApp() {
    try {
      setStatus("Loading…", false);
      if (initialLoader) initialLoader.style.display = "flex";

      // Initialize Parse dynamically (if loaded yet)
      ensureParse();
  ...
  // Ensure DOM is fully painted and loaded before launching script
